...
- For API Routes that handle external API calls, use a requiredUser() function to ensure that the user is authenticated before making the call. If requiredUser() fails, log the error and return a 401 Unauthorized response. If requiredUser() throws an error, catch the exception, log the stack trace, and return a 500 Internal Server Error response.
